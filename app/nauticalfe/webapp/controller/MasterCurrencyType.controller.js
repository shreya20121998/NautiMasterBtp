  ...
  function (Controller, History, Fragment, MessageToast, MessageBox, JSONModel) {
    ...
    return Controller.extend("nauticalfe.controller.MasterCurrencyType", {
      ...
      onInit: function () {
        ...
        oModel.loadData("/odata/v4/nautical/ZBTP_NAUTICAL_CURRENCY",[filter])
                          console.log(oModel);
      ...
      showValueHelpCurrency1: function () {
        // Create a reference to 'this' for later use
        var that = this;
        ...
        var oDialog = new sap.m.Dialog({
            ...
            content: [
                ...
                new sap.m.Table({
                    id: "valueHelpTable", // Assign an ID to access later
                    ...
                    selectionChange: function (oEvent) {
                        ...
                        var oSelectedDescription = oSelectedItem.getCells()[1].getText(); // Get the description
                        var inputCurrType = that.getView().byId("NAVOYCUR2"); // Input field for Voyage Type
                        var inputCurrDesc = that.getView().byId("NAVOYGCURDES2"); // Input field for Description
                        that.populateInputField(inputCurrType, oSelectedValue);
                        that.populateInputField(inputCurrDesc, oSelectedDescription); // Populate Description field
                        oDialog.close();
                    },
                })
            ],
        ...
        let oValueHelpTable = oDialog.getContent()[1]; // Access the Table
        oValueHelpTable.bindItems({
            path: "/ZBTP_NAUTICAL_CURRENCY", // Replace with your entity set
            template: new sap.m.ColumnListItem({
                cells: [
                    new sap.m.Text({ text: "{waers}" }),
                    new sap.m.Text({ text: "{ltext}" }),
                ],
            }),
        });
